...
void FOC_Control_Current_ISR(FOC_Controller_t *foc, float current_a, float current_b, float vbus, float temp_fet, float dt)
{
    ...
    // ---- Open-Loop Fallback Anti-Stall ----
    // When the cycloidal gearbox traps the rotor at a high-friction position,
    // closed-loop FOC produces a static DC voltage vector that cannot break
    // static Coulomb friction. Solution: seamlessly switch to the proven
    // open-loop V/f drive (Vd=9V, angle advancing at target speed) to generate
    // a traveling wave that breaks through any gear tooth bind, then recover
    // back to closed-loop when the motor is spinning again.
    static float ol_stall_time = 0.0f;
    static float ol_angle = 0.0f;
    static float ol_recovery_time = 0.0f;
    static bool  ol_active = false;

    float mech_rpm_fb = fabsf(omega_mech * (60.0f / (2.0f * (float)M_PI)));
    ...
    if (ol_active) {
        /* Open-Loop V/f Fallback: identical physics to the proven open-loop drive */
        ...
        ol_angle += target_elec_rad_s * dt_fast;
        utils_norm_angle_rad(&ol_angle);

        float v_boost = 9.0f;
        float v_bemf = conf_now->foc_motor_flux_linkage * fabsf(target_elec_rad_s);
        ...
        float sin_ol, cos_ol;
        sincos_lut(ol_angle, &sin_ol, &cos_ol);
        ...
        state_m->mod_q = 0.0f;
        state_m->mod_alpha_raw = cos_ol * v_norm;
        state_m->mod_beta_raw  = sin_ol * v_norm;
        ...
        /* Reset speed integrator each cycle to prevent windup during fallback */
        motor->m_speed_i_term = 0.0f;

        uint32_t ta_ol, tb_ol, tc_ol, sector_ol;
        foc_svm(state_m->mod_alpha_raw, state_m->mod_beta_raw,
                conf_now->l_max_duty, 1000, &ta_ol, &tb_ol, &tc_ol, &sector_ol);

        float duty_a_ol = (float)ta_ol / 1000.0f;
        float duty_b_ol = (float)tb_ol / 1000.0f;
        float duty_c_ol = (float)tc_ol / 1000.0f;

        foc->duty_a = duty_a_ol;
        if (foc->phase_swap_bc) {
            foc->duty_b = duty_c_ol;
            foc->duty_c = duty_b_ol;
        } else {
            foc->duty_b = duty_b_ol;
            foc->duty_c = duty_c_ol;
        }
        return; /* Skip closed-loop PI and modulation entirely */
    }
